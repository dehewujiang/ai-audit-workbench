...
import { Project, AppState, ProjectContextType } from '../types';
...
useEffect(() => {
    ...
    // 确定存储 key
    const storageKey = user.id === 'guest-user-001' ? 'guest-projects' : `user-projects-${user.id}`;
    ...
    if (savedProjects) {
      ...
      // 同步更新内存中的 projects 数组
      setProjects(prev => prev.map(p => p.id === activeProject.id ? updatedProject : p));
      
      console.log(`[ProjectContext] Saved project ${activeProject.id} to localStorage`);
    }
  }, [
    // 聊天相关
    chatCtx.messages,
    // 审计程序相关
    auditCtx.auditPrograms,
    auditCtx.activeProgramId,
    auditCtx.draftProgram,
    // 审计发现相关
    auditCtx.findings,
    // 舞弊分析相关
    auditCtx.fraudAnalyses,
    auditCtx.currentFraudPlan,
    auditCtx.lastFraudAnalysisResult,
    // 挑战模式相关
    auditCtx.currentChallengePlan,
    auditCtx.lastChallengeResult,
    // 报告相关
    auditCtx.generatedReportContent,
    auditCtx.reportGenerationTitle,
    auditCtx.currentReportPlan,
    auditCtx.pendingReportConfig,
    // 发现分析相关
    auditCtx.currentFindingAnalysisPlan,
    auditCtx.pendingFindingData,
    // 引导数据
    auditCtx.collectedGuidanceData,
    auditCtx.guidanceStage,
    // UI 状态
    auditCtx.activeTab,
    auditCtx.pinnedFileIds,
    // 脱水上下文
    auditCtx.distilledContext,
    // 上下文依赖
    activeProject,
    user,
  ]);
    ...
    const handleCreateProject = async (name: string) => {
      ...
      const userProjects = existing ? JSON.parse(existing) : [];
...
const switchProject = (id: string) => {
    ...
    if (activeProject && user) {
      console.log(`[ProjectContext] Preparing to switch from ${activeProject.id} to ${id}`);
      ...
      if (savedProjects) {
        ...
        const newProjects = parsed.map(p => p.id === activeProject.id ? updatedProject : p);
        localStorage.setItem(storageKey, JSON.stringify(newProjects));
        console.log(`[ProjectContext] Saved current project ${activeProject.id} before switch`);
      }
    ...
    if (targetProject) {
      ...
      // 保存当前活跃项目ID
      if (user) {
        const activeKey = user.id === 'guest-user-001' ? 'guest-activeProjectId' : `user-activeProjectId-${user.id}`;
        localStorage.setItem(activeKey, id);
        console.log(`[ProjectContext] Switched to project ${id}, saved active project ID`);
      }
    }
  };
